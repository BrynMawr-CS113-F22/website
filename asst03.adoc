...
consisting of spaces and asterisks.
...
The ISBN number is 10-digit code that uniquely specifies a book. For example, 
the ISBN number for _Harry Potter and the Socerer's Stone_ is 9780747532743.

The right-most digit is a _checksum_ digit that can be computed using the other 9 digits. 
Suppose we have an ISBN number, defined as stem:+d_{10} d_9 d_8 d_7 d_6 d_5 d_4 d_3 d_2 d_1+, 
where each stem:+d_i+ is a digit with value between 0 and 9. For example, 
for _Harry Potter_, d_{10} is 9; d_9 is 7; d_8 is 8; etc.

The ISBN number has the property that the following expression must be a multiple of 11.

[stem]
++++
10d_{10} + 9d_9 + 8d_8 + 7d_7 + 6d_6 + 5d_5 + 4d_4 + 3d_3 + 2d_2 + d_1
++++

Therefore, to find the checksum you should test each digit, from 0 to 9, to find 
a value for d_1 that results in a number that is a multiple of 11. Below are 
examples of running this program. User input is in bold.
